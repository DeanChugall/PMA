...
class TestUsersAndProfiles:
    ...
    def test_kreiranje_jednog_no_auth_vozaca(
        self, novi_jedan_vozac_ne_autorizovan_korisnik_fixture
    ):
        ...
        assert (
            novi_jedan_vozac_ne_autorizovan_korisnik_fixture.username
            != pogresan_username
        )

    def test_kreiranje_jednog_auth_vozaca(
        self, novi_jedan_vozac_autorizovan_korisnik_fixture
    ):
        assert Vozac.objects.count() == 1
    ...
    def test_provera_id_u_profilu_i_vozacu(
        self, novi_jedan_vozac_ne_autorizovan_korisnik_fixture
    ):
        ...
        profil_vozaca = VozacProfile.objects.get(user_id=id_user_vozaca_from_db)
    ...
    def test_provera_polja_profila_vozaca(
        self, novi_jedan_vozac_ne_autorizovan_korisnik_fixture
    ):
        ...
        profil_vozaca.save()
        broj_telefona_iz_profila = VozacProfile.objects.get(
            broj_telefona="+381 63 111 222 333"
        )
